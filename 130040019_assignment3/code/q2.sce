...
selected_order_list = [10,6,14,20]
...
//for iii = 1:length(Fs_list)
        ...
        n_fft = 10*length(x)
                ...
                ceptral_coefs = find_ceptral_coefs(y',n_fft)
               ...
               fig = scf()
               plot(ceptral_coefs)
               plot_title = strcat(['cepstral coefficients for ,',files_list(iii),' for p = ',string(selected_order_list(iii))])
               xtitle(plot_title,'n','c[n]')
               legend (['Original Signal','LP envelope','LP reconstructed','Cepstral Estimate'],3)
               ...
               //H_mag = abs(H(1:n_fft/2))          
               freq_array = linspace(0,Fs/2,n_fft/2)
               plot(freq_array,20*log(H_mag),'cyan')

               //Ceptral
               H = fftshift(fft(padded_ceptral_coefs))    
               H_mag = abs(H(n_fft/2 +1:n_fft))          
               freq_array = linspace(0,Fs/2,n_fft/2)
               ceptral_fft = abs(exp(H(n_fft/2 +1:n_fft)))
               plot(freq_array,20*log(ceptral_fft),'k')
               
               plot_title = strcat(['Cepstral estimationn and LP approximation for synthetic ',files_list(iii),' for p = ',string(selected_order_list(iii))])
